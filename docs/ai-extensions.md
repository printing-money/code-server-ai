...
Codex and Claude Code are installed from the official VS Code Marketplace. The
tested versions, required host commands, and credential locations live in
`compatibility/ai-extensions.json`; no modified extension bundle is included.
...
into an image. Keep outbound HTTPS available for Marketplace downloads and AI
API requests.
